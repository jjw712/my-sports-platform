...
export class PostsService {
  ...
  async list(take: number, cursor?: number) {
    ...
    if (ids.length === 0)
      return posts.map((p) => ({ ...p, likeCount: 0, dislikeCount: 0 }));
  ...
  create(dto: CreatePostDto, user: AuthUser) {
  ...
  async toggleReaction(
    postId: number,
    dto: { userId: string; type: 'LIKE' | 'DISLIKE' },
  ) {
    ...
    const [likeCount, dislikeCount] = await Promise.all([
      this.prisma.client.postReaction.count({
